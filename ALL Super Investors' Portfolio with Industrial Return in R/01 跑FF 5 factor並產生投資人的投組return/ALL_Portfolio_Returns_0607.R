...
#讀檔
setwd("/Users/Andy 1/Google 雲端硬碟 (r08323004@g.ntu.edu.tw)/0 Semesters/108-2/四234 文字探勘/0 中信 投資因子分析/crawling/Bill Ackman/ALL Super Investors' Portfolio with Industrial Return in R/01 跑FF 5 factor並產生投資人的投組return")
...
#S&P 500's return
#SP500 = read.csv("raw data/^GSPC.csv")
#SP500$Date = as.Date(SP500$Date)
#rSP500 = SP500%>%tq_transmute(select = Close,
...
###############################
investor_returns = list() #把super investors的portfolio return存起來
...
for(file in filenames){
  ...
  write.table(m_weight, out_path_weight, sep = ',', row.names = F)
  write.table(rdf, out_path_return, sep = ',', row.names = F)
  
  #拿掉日期前，另存日期為vector
  #date_preserve = rdf$Date
  investor_return = rdf[,1]
  ...
  #把Rp丟回有日期的dataframe
  investor_return = cbind(investor_return, Rp)
  #改colname
  colnames(investor_return) = c("Date", short_filename)
  #設定成DF
  investor_return = as.data.frame(investor_return)
  investor_return$Date = as.Date(investor_return$Date)
  #存成大的list: 所有investor的return
  investor_returns[length(investor_returns)+1] = list(investor_return) #append進list
  ...
  #依據大盤績效取特定期間的資料
  #FF_SP = FF[FF$rSP500>-0.03 & FF$rSP500<0.02,]
  FF_SP = FF
  
  #Regression
  try({ 
    reg1 = lm(Rp.Rf~Mkt.RF+NASDQ,
              data = FF_SP)
    summary(reg1)
    reg2 = lm(Rp.Rf~Mkt.RF+NASDQ+SMB+HML,
              data = FF_SP)
    summary(reg2)
    reg3 = lm(Rp.Rf~Mkt.RF+NASDQ+SMB+HML+RMW+CMA,
              data = FF_SP)
    summary(reg3)
    
    reg4 = lm(Rp.Rf~Mkt.RF+NASDQ+SMB+HML+RMW+CMA+R_ME,
              data = FF_SP)
    summary(reg4)
    reg5 = lm(Rp.Rf~Mkt.RF+NASDQ+SMB+HML+RMW+CMA+R_IA,
              data = FF_SP)
    summary(reg5)
    reg6 = lm(Rp.Rf~Mkt.RF+NASDQ+SMB+HML+RMW+CMA+R_ROE,
              data = FF_SP)
    summary(reg6)
    reg7 = lm(Rp.Rf~Mkt.RF+NASDQ+SMB+HML+RMW+CMA+R_ME+R_IA+R_ROE,
              data = FF_SP)
    summary(reg7)
    
    #reg out
    reg_out_file = "reg output/regression table/"+short_filename+"_reg.html"
    x = tab_model(reg1, reg2, reg3, reg4, reg5, reg6, reg7, show.p = T, file = reg_out_file)
    cat(x[["page.style"]], x[["page.complete"]], file = reg_out_file)
    reg_out_png = "reg output/regression table/png/"+short_filename+"_reg.png"
    webshot(reg_out_file, reg_out_png) #save as png
  })
}

# 把每個投資人的portfolio return放在一張表
rInvestors = investor_returns[[1]]

for(investor in investor_returns[2:length(investor_returns)]){
  rInvestors = merge(rInvestors, investor, by = "Date", all = T)
}
write.table(rInvestors, "raw data/rInvestors.csv", sep = ',', row.names = F)

# 其他關於return的程式
plot(fama$HML[fama$Date >= "2000-01-01"], fama$NASDQ[fama$Date >= "2000-01-01"])
cor(data.frame(fama$HML[fama$Date >= "2000-01-01"], fama$NASDQ[fama$Date >= "2000-01-01"]))
cor(fama[269:681,c(4,14)])

reg.NASDQ = lm(NASDQ~Mkt.RF+SMB+HML+RMW+CMA,
               data = fama)
summary(reg.NASDQ)


#dim(fama[fama$rSP500>-0.02 & fama$rSP500<0.02 &fama$Date>"2012-01-01",])
#dim(fama[fama$rSP500>0.02&fama$Date>"2012-01-01",])
#dim(fama[fama$rSP500<-0.02&fama$Date>"2012-01-01",])

#dim(FF)
#hist(fama$rSP500, ylim = c(0, 100), breaks = 50, xlim = c(-0.05,0.05))

#fama[fama$Mkt.RF<5 & fama$Mkt.RF>0,]
#fama$Mkt.RF[fama$Mkt.RF<5 & fama$Mkt.RF>0]
#cat(x[["page.style"]], x[["page.complete"]], file = reg_out_file)
